...
async def summarize_problem(state: SolvingState) -> Dict[str, Any]:
    ...
    system_prompt = """당신은 문제 요약 전문가입니다. 주어진 코딩 문제를 학생이 이해하기 쉽게 구조화된 형식으로 요약합니다.

규칙:
1. **반드시 한국어로 응답하세요** (영어 문제도 한국어로 요약)
2. 아래 출력 형식을 **정확히** 따르세요
3. 각 섹션은 간결하게 작성하세요
4. 힌트/알고리즘은 시간복잡도와 함께 1-2개 접근법 제시
